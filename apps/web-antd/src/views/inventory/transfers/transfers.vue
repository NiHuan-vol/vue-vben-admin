 ...
 * @version: V1.0
 * @Author: GaoWei
 * @Date: 2021-07-14 16:54:31
 * @LastEditors: GW
 * @LastEditTime: 2021-09-27 17:28:30
-->
<template>
                    ...
                    <template slot="neworder">
                        <a-dropdown class="btns" key="new" style="display:block">
                            ...
                            <a-menu slot="overlay">
                                ...
                                <a-menu-item key="1">
                                    <a href="javascript:void(0)" @click="choiceBills('PurchaseReceiveVoucher', 'ischoose', '选采购入库')">选采购入库</a>
                                </a-menu-item>
                            </a-menu>
                        </a-dropdown>
                    </template>
                    <!-- <template slot="shengdan">
                        <a-dropdown class="btns" key="sheng" style="display:block">
                            ...
                            <a-menu v-if="voucherState==1" slot="overlay">
                                <a-menu-item key="0">
                                    <a href="javascript:void(0)" @click="generateBills('PayableVoucher')">生成应付单</a>
                                </a-menu-item>
                            </a-menu>
                        </a-dropdown>
                    </template> -->
            ...
            <div ref="print" class="documents-wrapper" id="documents-wrapper">
                ...
                <section v-show="showModel=='order'">
                    <div>
                        ...
                        <span class="audit-status" v-show="voucherState==2">已批准</span>
                        <span class="audit-status" v-show="voucherState==3">已调拨</span>
                    </div>
...
import { save, examine, remove, getCode, allot, approval, returnBack } from '@/api/inventory/transfers';
...
export default {
    ...
    data () {
        return {
            ...
            basePostData: {
                ...
                startVoucherState: 1      //开始状态
            },
            sourceCode: 'InventoryAllocation',
            reportCodeDetail: 'InventoryAllocation_Detail_Report',          //明细code
            reportCodeStatistical: 'InventoryAllocation_Statistic_Report',   //统计code
            reportCodeList: 'InventoryAllocation_List_Report',//列表code
    ...
    methods: {
        ...
        returnBack () {
            ...
            returnBack(requestData).then((response) => {
                if (response.code === 200) {
                    const backData = response.data;
                    this.$message.success('退单成功');
                    this.getIdByData(null, 'transfers');
                } else {
                    // this.$message.warning(response.message);
                }
            })
