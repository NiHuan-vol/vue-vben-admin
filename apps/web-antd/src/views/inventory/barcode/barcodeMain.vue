          ...
          <a-col :xs="24" :sm="24" :md="24" :lg="16" :xl="12">
            ...
            <a-col :span="2" v-else>
              <div class="btns" @click="changeView('order')">
                <Icon class="btns-icon" icon="form" />
                <div class="btns-text">表单</div>
              </div>
            </a-col>
          </a-col>
        </a-row>
            ...
            <div class="header-box">
              <a-row class="order-dynamic-form-wrapper">
                <a-col :xs="grid.xs" :sm="grid.sm" :md="grid.md" :lg="grid.lg" :xl="grid.xl" :xxl="grid.xxl">
                  <a-form-model-item :label-col="labelCol" :wrapper-col="wrapperCol" label="条码编码" :rules="[{ required: true, message: '', trigger: 'input' }]">
                    <a-input v-model.trim="requestData.code"></a-input>
                  </a-form-model-item>
                </a-col>
                <a-col :xs="grid.xs" :sm="grid.sm" :md="grid.md" :lg="grid.lg" :xl="grid.xl" :xxl="grid.xxl">
                  <a-form-model-item :label-col="labelCol" :wrapper-col="wrapperCol" label="条码名称" :rules="[{ required: true, message: '', trigger: 'input' }]">
...
import { listAttr, saveAttr, deleteAttr } from '@/api/inventory/barcode';
import EditableCell from './editTable/EditableCell.vue';
import { Icon } from '@/components/Icon';

// 表格列定义
const columns = [
  { title: '条码编码', dataIndex: 'code' },
...
const requestData = ref({ code: '', name: '', remark: '', type: 1 });
...
const handleAdd = () => {
  ...
  requestData.value = { code: '', name: '', remark: '', type: 1 };
