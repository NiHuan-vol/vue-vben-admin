          ...
          <template slot="neworder">
            <a-dropdown class="btns" key="new" style="display:block">
              ...
              <a-menu slot="overlay">
                ...
                <a-menu-item key="1">
                  <a href="javascript:void(0)" @click="choiceBillsFromSaleDelivery">选退货通知单</a>
                </a-menu-item>
              </a-menu>
            </a-dropdown>
          </template>
      ...
      <div ref="print" class="documents-wrapper" id="documents-wrapper">
        ...
        <section v-show="showModel==='order'">
          <div>
            ...
            <span class="audit-status" v-show="voucherState===1 || voucherState===10">{{voucherState===1?'待入库':'关闭'}}</span>
...
import { saveSaleOutRet, getSaleOutRetDetail, deleteSaleOutRet, auditSaleOutRet, submitSaleOutRetOrder, exportSaleOutRet } from '/@/api/inventory/bills';
...
const voucherTitle = ref<string>('销售退货单');
...
function submit(requestData: any) {
  ...
  saveSaleOutRet(requestData).then(response => {
    loading.value = false;
...
function submitOrder(type: number) {
  ...
  loading.value = true;
  submitSaleOutRetOrder(data).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
function getIdByData(id?: string) {
  ...
  loading.value = true;
  getSaleOutRetDetail(orderId.value).then(response => {
    loading.value = false;
...
function deleteOrder() {
  ...
  loading.value = true;
  deleteSaleOutRet(orderId.value).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
function auditClick(type: number) {
  ...
  loading.value = true;
  auditSaleOutRet(data).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
function exportDetail() {
  ...
  exportSaleOutRet(params).then((response) => {
    loading.value = false;
    if (response) {
      ...
      a.href = url;
      a.download = `销售退货单_${formatDate(new Date())}.xlsx`;
