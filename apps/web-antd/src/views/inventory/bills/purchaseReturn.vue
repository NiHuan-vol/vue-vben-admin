          ...
          <template slot="neworder">
            <a-dropdown class="btns" key="new" style="display:block">
              ...
              <a-menu slot="overlay">
                ...
                <a-menu-item key="1">
                  <a href="javascript:void(0)" @click="choiceBillsFromPurchaseReceiveVoucher">选采购入库单</a>
                </a-menu-item>
              </a-menu>
            </a-dropdown>
          </template>
...
import { savePurchaseReturn, getPurchaseReturnDetail, deletePurchaseReturn, auditPurchaseReturn, submitPurchaseReturnOrder, generateBills } from '/@/api/inventory/bills';
...
const voucherTitle = ref<string>('采购退货单');
...
const sourceCode = ref<string>('PurchaseReceiveVoucher');
const reportCodeDetail = ref<string>('PurchaseReceiveVoucher_Detail_Report');
const reportCodeStatistical = ref<string>('PurchaseReceiveVoucher_Statistic_Report');
const reportCodeList = ref<string>('PurchaseReceiveVoucher_List_Report');
const receiveDeliverType = ref<number>(2); // 1入，2出
const businessType = ref<number>(1);
const isHideWherehouse = ref<boolean>(true);
const checkType = ref<string>('is_purchasing'); // 采购
...
function initForm(type: number) {
  ...
  orderHeaderData.value = [
    ...
    { label: '供应商', field: 'supplierName', value: '', required: true },
    { label: '供应商编码', field: 'supplierCode', value: '' },
    { label: '采购员', field: 'buyerName', value: '' },
...
function submit(requestData: any) {
  ...
  savePurchaseReturn(requestData).then(response => {
    loading.value = false;
...
function submitOrder(type: number) {
  ...
  loading.value = true;
  submitPurchaseReturnOrder(data).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
function getIdByData(id?: string) {
  ...
  loading.value = true;
  getPurchaseReturnDetail(orderId.value).then(response => {
    loading.value = false;
...
function deleteOrder() {
  ...
  loading.value = true;
  deletePurchaseReturn(orderId.value).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
function auditClick(type: number) {
  ...
  loading.value = true;
  auditPurchaseReturn(data).then(response => {
    loading.value = false;
    if (response.code === 200) {
...
// 选采购入库单
function choiceBillsFromPurchaseReceiveVoucher() {
  // 打开选择采购入库单的抽屉
