...
import {
  saveMaterialOutRet,
  examineMaterialOutRet,
  deleteMaterialOutRet,
  getMaterialOutRetCode,
  confirmMaterialOutRet,
  submitMaterialOutRet,
} from '@/api/inventory/materialOutRet';
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive<{
      ...
      businessType: number;
      isPRF: boolean;
      headerInBusinessType: boolean;
      isHideWherehouse: boolean;
      checkType: string;
      productCheckType: string;
      showModel: string;
      editMask: boolean;
      loading: boolean;
      voucherState: number;
      ...
      businessType: 5, // 业务类型单独处理
      isPRF: true,
      headerInBusinessType: true, // 表头业务类型不可修改
      isHideWherehouse: true, // 隐藏明细当中的仓库和仓库编码
    ...
    const submit = (requestData: any) => {
      ...
      saveMaterialOutRet(requestData)
        .then((response: any) => {
    ...
    const submitOrder = (type: number) => {
      ...
      submitMaterialOutRet(data).then((res: any) => {
        if (res.code === 200) {
          if (type === 0) {
    ...
    const auditClick = (type: number) => {
      ...
      examineMaterialOutRet(requestData).then((response: any) => {
        if (response.code === 200) {
          if (type === 1) {
    ...
    const remove = (postData: any) => {
      deleteMaterialOutRet(postData).then((response: any) => {
        if (response.code === 200) {
    ...
    const getCode = () => {
      ...
      getMaterialOutRetCode(data).then((res: any) => {
        if (res.code === 200) {
    ...
    const theGoods_delete = () => { // 废弃方法
      ...
      _this.$confirm({
        ...
        onOk() {
          ...
          confirmMaterialOutRet(requestData).then((response: any) => {
            if (response.code === 200) {
