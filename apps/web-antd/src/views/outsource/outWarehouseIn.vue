<!--
 * @Descripttion: 委外管理，委外入库
 * @version: V1.0
 * @Author: GaoWei
 * @Date: 2021-04-25 12:11:56
 * @LastEditors: GW
 * @LastEditTime: 2021-12-21 11:05:30
-->
<template>
  <div class="out-warehouse-in-container">
    <Card :title="voucherTitle" class="mb-4">
      ...
      <template #extra>
        <div class="flex space-x-2">
          ...
          <a-dropdown v-if="voucherState === 1 || voucherState === 2">
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item key="0" @click="generateBills('DelegateInvoice')">
                  生成委外发票
                </a-menu-item>
              </a-menu>
            </template>
      ...
      <div class="mb-4">
        ...
        <span v-else-if="voucherState === 2" class="inline-block px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">已入库</span>
...
import { save, examine, remove, getCode, submitOrder as apiSubmitOrder } from '@/api/inventory/inout';
...
const sourceCode = ref<string>('DelegateReceiveVoucher');
const voucherTitle = ref<string>('委外入库单');
const receiveDeliverType = ref<number>(1); // 1入，2出
...
const columns = [
  ...
  { title: '入库仓库', dataIndex: 'warehouseName', width: 150 },
...
// 保存
const handleSave = () => {
  if (!orderHeaderData.value.vendorName || !orderHeaderData.value.warehouseName) {
    message.warning('请填写委外供应商和入库仓库');
    return;
  }
  loading.value = true;
  const requestData = {
...
const loadOrderData = (id: string) => {
  ...
  setTimeout(() => {
    ...
    orderFooterData.value = {
      totalAmount: 20000,
      creator: '管理员',
      createTime: '2021-04-25 12:30:00',
      auditor: '审核员',
      auditTime: '2021-04-25 13:00:00',
    };
  }, 1000);
};
