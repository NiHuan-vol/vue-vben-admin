<template>
  <div class="content-wrapper a-base-container">
    <Card title="采购请购单">
      ...
      <div class="btn-wrapper flex flex-wrap gap-2 mb-4">
        ...
        <Dropdown placement="bottomRight" v-if="voucherState === 1">
          ...
          <template #overlay>
            <Menu>
              <MenuItem key="0" @click="generateBills('PurchaseOrder')">
                <span>生成采购订单</span>
              </MenuItem>
            </Menu>
          </template>
      ...
      <div v-else-if="showModel === 'order'" class="order-view">
        <div class="mb-2">
          ...
          <span v-else-if="voucherState === 10" class="audit-status已关闭">关闭</span>
        </div>
    ...
    <!-- 生单模态框 -->
    <RequisitionModal ref="purchaseOrderRef" @confirm="handleGenerateConfirm" />
  </div>
</template>

<script lang="ts" setup>
import { ref, reactive, onMounted, computed } from 'vue';
import { Card, Button, Dropdown, Menu, MenuItem, Spin, Modal } from 'ant-design-vue';
...
import { save, examine, remove, getCode, submitOrder, getList, getDetail } from '/@/api/purchase/requisitionOrder';
...
const orderId = ref('');
const sourceCode = ref('PurchaseRequisition');
const reportCodeDetail = ref('PurchaseRequisition_Detail_Report');
const reportCodeStatistical = ref('PurchaseRequisition_Statistic_Report');
const reportCodeList = ref('PurchaseRequisition_List_Report');
const checkType = ref('is_purchasing');
const businessType = ref('purchase');
const voucherTitle = ref('采购请购单');
...
const columns = ref<any[]>([
  { title: '请购单号', dataIndex: 'voucherCode', key: 'voucherCode', width: 140 },
  { title: '请购日期', dataIndex: 'voucherDate', key: 'voucherDate', width: 120, formatter: formatToDate },
...
const detailColumns = ref<any[]>([
  ...
  { title: '请购数量', dataIndex: 'quantity', key: 'quantity', width: 100, slot: 'quantity' },
  ...
  { title: '需求日期', dataIndex: 'demandDate', key: 'demandDate', width: 120, slot: 'demandDate', formatter: formatToDate },
...
const slotArray = ref<string[]>(['materialCode', 'materialName', 'specification', 'unitName', 'quantity', 'price', 'demandDate', 'remark']);
...
function handleSubmit() {
  ...
  // 处理明细数据
  requestData.orderDetailList.forEach((item: any) => {
    item.source = 'PurchaseRequisition';
    item.baseUnit = item.baseUnitId;
  });
...
function generateBills(type: string) {
  if (!orderId.value) {
    message.warning('请先选择请购单');
    return;
  }

  const purchaseOrderRef = document.querySelector('#purchaseOrderRef') as any;
  if (purchaseOrderRef) {
    purchaseOrderRef.showModal(type, orderId.value);
  }
}
...
// 处理选择添加数据
function handleChoiceAddData(index: number, col: string, data: any, type: string) {
  // 实现选择添加数据逻辑
}
...
// 生单确认
function handleGenerateConfirm(result: boolean) {
  if (result) {
