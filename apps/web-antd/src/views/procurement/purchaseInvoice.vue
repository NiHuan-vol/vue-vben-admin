    ...
    <div class="requisition-set-wrapper">
      ...
      <div class="btn-wrapper flex flex-wrap gap-2 mb-4">
        <Dropdown placement="bottomRight">
          ...
          <template #overlay>
            <Menu>
              ...
              <MenuItem key="1" @click="handleChoiceBillsFromPurchaseReceiveVoucher">
                <span>选采购入库</span>
              </MenuItem>
            </Menu>
          </template>
        ...
        <Dropdown placement="bottomRight" v-if="voucherState === 1">
          ...
          <template #overlay>
            <Menu>
              <MenuItem key="0" @click="generateBills('PayableVoucher')">
                <span>生成应付单</span>
              </MenuItem>
            </Menu>
          </template>
        ...
        <Button v-if="showModel === 'order' && voucherState === -1" type="success" @click="handleSubmitOrder(0)">提交</Button>
        <Button v-if="showModel === 'order' && voucherState === 0" type="warning" @click="handleSubmitOrder(-1)">撤回提交</Button>
      ...
      <div ref="print" class="documents-wrapper" id="documents-wrapper">
        ...
        <section v-show="showModel === 'order'">
          <div class="mb-2">
            ...
            <span v-if="voucherState === 1 || voucherState === 10" :class="voucherState === 1 ? 'audit-status已审核' : 'audit-status关闭'">
              {{ voucherState === 1 ? '已审核' : '关闭' }}
            </span>
          </div>
          ...
          <Form :layout="formLayout">
            <div class="header-box mb-4 p-3 bg-gray-50 rounded-md">
              <DynamicForm
                ...
                @clickItem="handleSetValue"
                @downSelectChange="handleDownSelectChange"
              />
                ...
                <template v-for="col in slotArray" :key="col" :slot="col">
                  <div data-key="">
                    <EditableCell
                      ...
                      @choiceAddData="handleChoiceAddData"
                      @change="handleCellChange"
                      @newAddData="handleAddRow"
                    />
                  </div>
                </template>
              </BasicTable>
...
import { Button, Dropdown, Menu, MenuItem, Spin, Form, Modal } from 'ant-design-vue';
...
import { save, examine, remove, getCode, submitOrder, getList, getDetail } from '/@/api/procurement/purchaseInvoice';
...
const orderId = ref('');
const sourceCode = ref('PurchaseInvoice');
const dataSourceCode = ref('PurchaseInvoice');
const reportCodeDetail = ref('PurchaseInvoice_Detail_Report');
const reportCodeStatistical = ref('PurchaseInvoice_Statistic_Report');
const reportCodeList = ref('PurchaseInvoice_List_Report');
const voucherTitle = ref('采购发票');
...
const businessType = ref(4); // 1:退货业务 2：非退货业务 3: 退货 + 非退货 4: 发票
...
const slotArray = ref<string[]>(['materialCode', 'materialName', 'specification', 'unitName', 'quantity', 'price', 'amount', 'taxAmount', 'totalAmount']);
...
const columns = ref<any[]>([
  ...
  { title: '数量', dataIndex: 'quantity', key: 'quantity', width: 100, slot: 'quantity' },
  ...
  { title: '税额', dataIndex: 'taxAmount', key: 'taxAmount', width: 100, formatter: formatMoney },
  { title: '价税合计', dataIndex: 'totalAmount', key: 'totalAmount', width: 120, formatter: formatMoney },
...
const basePostData = reactive<any>({
  ...
  current: 1,
  size: 10,
  someBusinessType: null, // 指定业务类型 默认 null
});
...
function generateBills(type: string) {
  ...
  if (type === 'PayableVoucher') {
    // 生成应付单
    message.info('生成应付单功能开发中');
  }
}
...
// 业务类型改变
function handleDownSelectChange(value: any) {
...
// 选采购入库
function handleChoiceBillsFromPurchaseReceiveVoucher() {
  const postData = {
    sourceCode: 'PurchaseReceiveVoucher', // 来源单
    title: '选采购入库',
    businessType: 4, // 1:退货业务 2：非退货业务 3: 退货 + 非退货 4: 发票
    someBusinessType: null, // 指定业务类型 默认 null
    startVoucherState: 0,
  ...
  if (searchListRef && searchListRef.onOpenBox) {
    searchListRef.onOpenBox(postData);
  }
}
