      ...
      <div ref="print" class="documents-wrapper" id="documents-wrapper">
        ...
        <section v-show="showModel === 'order'">
          <div class="mb-2">
            ...
            <span v-if="(voucherState === 1 || voucherState === 10) && !isSource" :class="voucherState === 1 ? 'audit-status已审核' : 'audit-status关闭'">
              {{ voucherState === 1 ? '已审核' : '关闭' }}
            </span>
            <span v-if="(voucherState === 1 || voucherState === 10) && isSource" :class="voucherState === 1 ? 'audit-status退货确认' : 'audit-status关闭'">
              {{ voucherState === 1 ? '退货确认' : '关闭' }}
            </span>
            <span v-if="voucherState === 2" class="audit-status已出库">已出库</span>
          </div>
...
import { FileAddOutlined, PrinterOutlined, ExportOutlined, ArrowLeftOutlined } from '@ant-design/icons-vue';
import { BasicTable } from '/@/components/Table';
import { useMessage } from '/@/hooks/web/useMessage';
import { save, examine, remove, getCode, submitOrder, getList, getDetail } from '/@/api/procurement/purchaseExamineReturn';
...
const voucherState = ref(-1); // -1: 未提交, 0: 待审核, 1: 已审核, 2: 已出库, 10: 关闭
const orderId = ref('');
...
const voucherTitle = ref('采购已入库退货单');
...
const businessType = ref(99); // 业务类型单独处理
...
const columns = ref<any[]>([
  ...
  { title: '退货数量', dataIndex: 'quantity', key: 'quantity', width: 100, slot: 'quantity' },
...
const basePostData = reactive<any>({
  ...
  current: 1,
  size: 10,
  someBusinessType: '99', // 指定业务类型 默认 0
});

// 选单查询数据
const choiceBillPostData = reactive<any>({
  sourceCode: 'PurchaseReceiveVoucher',
  title: '选采购入库单',
...
function getBillCode() {
  getCode({ sourceCode: sourceCode.value })
    .then((res) => {
      if (res.code === 200) {
