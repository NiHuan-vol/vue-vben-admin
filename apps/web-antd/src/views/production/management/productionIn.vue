      ...
      <div ref="printRef" class="documents-wrapper" id="documents-wrapper">
        ...
        <section v-show="showModel === 'order'">
          <div>
            ...
            <span class="audit-status" v-show="voucherState === 2">已入库</span>
          </div>
...
import { purchaseSave, examine, purchaseDelete, getCode, submitOrder } from '@/api/inventory/inout';
...
// 响应式数据
const sourceCode = ref('ProductReceiveVoucher');
const reportCodeDetail = ref('ProductReceiveVoucher_Detail_Report');
const reportCodeStatistical = ref('ProductReceiveVoucher_Statistic_Report');
const reportCodeList = ref('ProductReceiveVoucher_List_Report');
...
const handleSubmit = async (requestData: Record<string, any>) => {
  ...
  try {
    const response = await purchaseSave(requestData);
...
const handleAudit = async (type: number) => {
  ...
  if (type === 1) {
    Modal.confirm({
      ...
      content: (
        <div>
          <p>请审核<span style={{ color: 'red' }}>申请入库数量</span>是否正确</p>
          <p>申请入库后，此单自动进入库存管理中，是否申请入库？</p>
        </div>
      ),
      onOk: async () => {
        try {
...
const handleDelete = async (postData: Record<string, any>) => {
  try {
    const response = await purchaseDelete(postData);
    if (response.code === 200) {
