      ...
      <div ref="print" class="credentials-wrapper" id="documents-wrapper">
        <div style="min-height: 400px;">
          <h3 class="text-center">
            <span class="text-name">生产订单领料汇总表</span><br />
          </h3>
          <!-- 按钮 -->
...
function getList() {
  ...
  postAction('/production/work/pageProductionPickReturnCumSummaryVo', params)
    .then((response) => {
      loading.value = false;
...
function dataFormat(list: any[]) {
  ...
  list.forEach((item, index) => {
    const { officeId, officeName, inventoryId, inventoryCode, baseQuantity, plannedQuantity, cumPickQuantity, cumRepairQuantity, cumRetQuantity } = item;

    const plannedQuantityNum = plannedQuantity || 0;
    const cumPickQuantityNum = cumPickQuantity || 0;
    const cumRepairQuantityNum = cumRepairQuantity || 0;
    const cumRetQuantityNum = cumRetQuantity || 0;
    ...
    if (!sumInventory) {
      sumInventory = {
        ...
        cumPickQuantity: cumPickQuantityNum,
        cumRepairQuantity: cumRepairQuantityNum,
        cumRetQuantity: cumRetQuantityNum,
      };
    } else if (officeId === sumInventory.officeId && inventoryId === sumInventory.inventoryId) {
      sumInventory.baseQuantity = (sumInventory.baseQuantity || 0) + (baseQuantity || 0);
      sumInventory.plannedQuantity += plannedQuantityNum;
      sumInventory.cumPickQuantity += cumPickQuantityNum;
      sumInventory.cumRepairQuantity += cumRepairQuantityNum;
      sumInventory.cumRetQuantity += cumRetQuantityNum;
    } else {
      offSet++;
      sumInventory.baseQuantity = sumInventory.baseQuantity.toFixed(3) || 0;
      sumInventory.plannedQuantity = sumInventory.plannedQuantity.toFixed(3) || 0;
      sumInventory.cumPickQuantity = sumInventory.cumPickQuantity.toFixed(3) || 0;
      sumInventory.cumRepairQuantity = sumInventory.cumRepairQuantity.toFixed(3) || 0;
      sumInventory.cumRetQuantity = sumInventory.cumRetQuantity.toFixed(3) || 0;
    ...
    if (!sumOffice) {
      ...
      sumOffice.baseQuantity = (sumOffice.baseQuantity || 0) + (baseQuantity || 0);
      sumOffice.plannedQuantity += plannedQuantityNum;
      sumOffice.cumPickQuantity += cumPickQuantityNum;
      sumOffice.cumRepairQuantity += cumRepairQuantityNum;
      sumOffice.cumRetQuantity += cumRetQuantityNum;
    } else {
      offSet++;
      sumOffice.baseQuantity = sumOffice.baseQuantity.toFixed(3) || 0;
      sumOffice.plannedQuantity = sumOffice.plannedQuantity.toFixed(3) || 0;
      sumOffice.cumPickQuantity = sumOffice.cumPickQuantity.toFixed(3) || 0;
      sumOffice.cumRepairQuantity = sumOffice.cumRepairQuantity.toFixed(3) || 0;
      sumOffice.cumRetQuantity = sumOffice.cumRetQuantity.toFixed(3) || 0;
...
function handleNameChange() {
  // 物料名称搜索
  inputVal.value = inventoryName.value;
...
function handleEnter(t: number) {
  if (t === 1) {
    // 物料编码搜索处理
  } else if (t === 2 && searchArchivesRefName.value) {
...
function handlCloseArchives() {
  ...
  if (inputType.value === 1 || inputType.value === 2) {
    maskArchives.value = false;
    // 关闭物料编码搜索弹窗
