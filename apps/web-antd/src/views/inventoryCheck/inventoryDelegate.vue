<!--
 * @Descripttion: 委外加工入库核算
 * @version: V1.0
 * @Author: GaoWei
 * @Date: 2021-07-08 08:58:29
 * @LastEditors: GW
 * @LastEditTime: 2021-09-27 16:52:52
-->
<template>
    <div class="content-wrapper a-base-container">
        <div class="requisition-set-wrapper">
            ...
            <div class="btn-wrapper clearfix">
                <a-row class="text-center clearfix">
                    <a-col :xs="24" :sm="24" :md="24" :lg="16" :xl="12">
                        <a-col :xs="2" :sm="2" :md="2" :lg="2" :xl="2" v-if="permissions.includes('InventoryDelegate:audit')">
                        ...
                        <a-col :xs="2" :sm="2" :md="2" :lg="2" :xl="2" v-if="permissions.includes('InventoryDelegate:accounting')">
            ...
            <div ref="print" class="documents-wrapper" id="documents-wrapper">
                <h3 class="text-center">委外加工入库核算</h3>
                ...
                <section>
                    <a-table :data-source="dataSource" :columns="columns" :pagination="false" :scroll="{x:800,y:600}" bordered>
                        ...
                        <template v-for="item in ['price','amount']" :slot="item" slot-scope="text, record ,index">
                            <div :key="item">
...
import { checkOutColumnsJson, evaluateColumnsJson, selfRestraintColumnsJson, delegateColumnsJson, otherColumnsJson } from './columnsJson/checkOut'
export default {
    components: {
        editableCell,
        drawer
    ...
    data() {
        return {
            ...
            dataSource: [],
            showColumns: [],
            type: 4,//type:1 外购入库核算 type:2 存货暂估入库 type:3 产成品入库核算 type:4 委外加工入库核算 type:5 其他入库核算
            isRouterAlive: true,
            loading: false,
        }
    },
    ...
    methods: {
        ...
        getList() {
            let request = {
                dataSource: 'DelegateReceiveVoucher',
            }
            this.loading = true;
            gwtWarehousingListApi(request).then(res => {
                if (res.code === 200) {
        ...
        initColumns() {
            ...
            if (this.showColumns.length > 0) {
                delegateColumnsJson.forEach(item => {
